...
class WarpMetric:
    ...
    # ============================================================
    # МЕТОДИ ЗА СЕТ 06 - ЧИСЛЕНА СТАБИЛНОСТ
    # ============================================================
    
    def PhiX(self, x, y, z, t=None):
        if t is None:
            t = self.t
        h = self.params['eps_val']
    ...
    def PhiT(self, x, y, z, t=None):
        """Времева производна на Phi"""
        if t is None:
            t = self.t
        dt = 1e-5
        return (self.Phi(x, y, z, t+dt) - self.Phi(x, y, z, t-dt)) / (2*dt)
    
    def jacobian_geodesic(self, x, y, z):
        """Якобиан на системата от ODE за геодезични линии"""
        h = 1e-5
        dPhi_dx = self.PhiX(x, y, z)
    ...
    def geodesic_invariant(self, x, y, z, px, py, pz):
        """Релативистичен инвариант p_μ p^μ за фотон"""
    ...
    def lapse_boundary(self, x, y, z):
        """Проверка на lapse функцията α > 0"""
        alpha = self.alpha(x, y, z)
        return alpha


# ============================================================
# ПРОВЕРКИ 141-160: ЧИСЛЕНА СТАБИЛНОСТ, ОПТИМИЗАЦИЯ, ГЕОДЕЗИЧНИ
# ============================================================

class NumericalChecks:
    """Проверки 141-160: Числена стабилност, оптимизация и геодезични"""
    ...
    def check_141_144_jacobian(self):
        """Проверки 141-144: Якобиан на системата ODE за геодезични линии"""
        test_points = [
            (0, 0, 0, "център"),
    ...
    def check_145_148_geodesic_invariant(self):
        """Проверки 145-148: Запазване на p_μ p^μ = 0 за фотони"""
        test_points = [
            (0, 0, 0, "център"),
            (1.5, 0, 0, "вътрешен ръб"),
            (2.0, 0, 0, "стена"),
    ...
    def check_149_152_optimization_gradients(self):
        """Проверки 149-152: Градиенти в оптимизационни алгоритми"""
        test_points = [
            (0, 0, 0, "център"),
            (1.5, 0, 0, "вътрешен ръб"),
            (2.0, 0, 0, "стена"),
    ...
    def check_153_156_regularization(self):
        """Проверки 153-156: Регуларизация при r→0 (без NaN или ZeroDivision)"""
        test_points = [
            (self.metric.params['eps_val'], 0, 0, "близо до 0"),
    ...
    def check_157_160_lapse_boundary(self):
        """Проверки 157-160: Гранични стойности на lapse α = e^Φ (α > 0)"""
...
def main():
    ...
    print("="*70)
    print("WARP BUBBLE CHECKS - SET 06 (Проверки 141-160)")
    print("Числена стабилност, оптимизация и геодезични линии")
    print("="*70)
    print()
    ...
    filename = f"checks_set06_{timestamp}.json"
    ...
    report = {
        ...
        'set': "06",
        'checks': "141-160",
        'description': "Numerical stability, optimization and geodesic lines",
        'total': len(results),
        'passed': passed,
        'failed': failed,
